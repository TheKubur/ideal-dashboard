...
with open('js/ui/kanban.js', 'r', encoding='utf-8') as f:
    code = f.read()

# Replace renderActivities
def repl_activities(match):
    return """function renderActivities() {
...
code = re.sub(r'function renderActivities\(\) \{[\s\S]*?\n\}\n', repl_activities, code)

# Replace renderCRM for pagination
def repl_crm(match):
    return """
  if (!filtered.length) {
...
code = re.sub(r'  if \(\!filtered\.length\) \{[\s\S]*?    \}\);\n  \}', repl_crm, code)

with open('js/ui/kanban.js', 'w', encoding='utf-8') as f:
    f.write(code)
